package com.senok.coreeventpublisher.constants

object ConsumerGroupConstants {
    const val DEVICE_EVENT_CONSUMER_GROUP = "device.event.consumer.group"
    const val COUPLE_EVENT_CONSUMER_GROUP = "couple.event.consumer.group"
    const val COUPLE_REQUEST_CONSUMER_GROUP = "couple.request.event.consumer.group"
    const val COUPLE_MESSAGE_CONSUMER_GROUP = "couple.message.event.consumer.group"
}