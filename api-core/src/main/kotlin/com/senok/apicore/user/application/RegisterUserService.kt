...
class RegisterUserService(
    ...
    override fun registerUser(command: com.senok.apicore.user.application.`in`.command.RegisterUserCommand, userId: Long) {
        Tx.writable {
            val user = findUserPort.findUser(userId)

            user.activeUser(command.nickname, command.genderType)
            val device = com.senok.apicore.user.domain.user.Device.register(userId, UUID.randomUUID().toString(), command.providerType)
